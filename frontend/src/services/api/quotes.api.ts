...
import { Quote, CreateQuotePayload, QuoteLineItem, QuoteStatus } from '../../types/quote';
...
function adaptServerQuote(raw: any): Quote {
  ...
  const totalAmount = Number(raw.total || raw.totalAmount) || Math.max(0, subtotal - discountAmount + taxAmount);
  const discountPercentage = subtotal > 0 ? (discountAmount / subtotal) * 100 : 0;
  const totalCost = Number(raw.total_cost || raw.totalCost) || Math.round(subtotal * 0.72);
  const grossMarginAmount = totalAmount - totalCost;
  const grossMarginPercentage = totalAmount > 0 ? (grossMarginAmount / totalAmount) * 100 : 25;

  const lines: QuoteLineItem[] = Array.isArray(raw.lines) && raw.lines.length > 0
    ? raw.lines.map((l: any, idx: number) => {
        const qty = Number(l.quantity) || 1;
        const unitPrice = Number(l.unit_price || l.unitPrice) || 50000;
        const discPct = Number(l.discount_percent || l.discountPercentage) || 0;
        const lineSubtotal = qty * unitPrice;
        const discAmt = lineSubtotal * (discPct / 100);
        const lineTotal = Number(l.line_total || l.lineTotal) || (lineSubtotal - discAmt);
        const unitCost = Number(l.unit_cost || l.unitCost) || Math.round(unitPrice * 0.7);

        return {
          id: l.id || `line-${idx + 1}`,
          productId: l.product_id || l.productId || `prod-${idx + 1}`,
          productName: l.product_name || l.productName || 'Enterprise System Module',
          productSku: l.sku || l.productSku || `SKU-MOD-${100 + idx}`,
          categoryId: l.category_id || l.categoryId || 'cat-hardware',
          categoryName: l.category_name || l.categoryName || 'Enterprise Hardware',
          quantity: qty,
          unitPrice,
          ...
          lineMarginPercentage: lineTotal > 0 ? ((lineTotal - (qty * unitCost)) / lineTotal) * 100 : 25,
          isRecurring: false,
          stockAvailable: 100,
        };
      })
  ...
  const riskAssessment = calculateRiskAssessment(lines, subtotal, discountAmount, totalCost, grossMarginPercentage);

  return {
    id: raw.id || `q-${Date.now()}`,
    quoteNumber: raw.quote_number || raw.quoteNumber || `Q-${Math.floor(1000 + Math.random() * 9000)}`,
    ...
    salespersonName: raw.salesperson_name || raw.salespersonName || 'Ananya Sharma',
    currency: 'INR',
    priceListId: 'pl-default',
    status: (raw.status || 'DRAFT') as QuoteStatus,
    validUntil: raw.valid_until || raw.validUntil || new Date(Date.now() + 30 * 86400000).toISOString(),
    ...
    try {
      ...
      if (response.data && response.data.success && Array.isArray(response.data.data)) {
        ...
        if (search) {
          ...
          list = list.filter(item => item.quoteNumber.toLowerCase().includes(q) || item.customerName.toLowerCase().includes(q) || item.salespersonName.toLowerCase().includes(q));
        }
...
<<<<<<< HEAD
      // Calculate totals
=======
>>>>>>> 2175615b0bb88e4604023895a3ed4d5771f59e7b
      let subtotal = 0;
      let discountTotal = 0;
